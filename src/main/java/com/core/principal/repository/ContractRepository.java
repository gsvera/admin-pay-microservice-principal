...
public interface ContractRepository extends JpaRepository<Contract, Long> {
    ...
    @Query(value = "" +
            "SELECT " +
            "c.id, c.id_customer, c.folio, c.status_contract, cu.first_name, cu.last_name, p.pay_date, p.pay_amount " +
            "FROM tbl_contract c " +
            "LEFT JOIN tbl_customer cu ON c.id_customer = cu.id " +
            "LEFT JOIN (SELECT id_contract, pay_date, pay_amount FROM tbl_pay ORDER BY id DESC LIMIT 1) p ON c.id = p.id_contract " +
...
//    @Query(value = "SELECT c.* FROM tbl_contract c LEFT JOIN tbl_pay_contract p ON c.id = p.id_contract WHERE c.id = ?1", nativeQuery = true)
    @Query(value = "SELECT c FROM Contract c LEFT JOIN FETCH c.paysAmortization WHERE c.id = ?1")
    Contract findByIdWithPays(@Param("id") Long id);
    @Modifying
    @Transactional
    @Query(value = "UPDATE tbl_contract SET status_contract = ?1 WHERE id = ?2", nativeQuery = true)
    int updateStatusContract(int statusNumber, Long idContract);
}
